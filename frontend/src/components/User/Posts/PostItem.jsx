    ...
    return (
        <>
            <div onClick={() => setOpen(true)} className="group w-full h-32 sm:h-72 max-h-72 flex justify-center items-center bg-black cursor-pointer relative z-0">
                <img draggable="false" loading="lazy" className="hover:opacity-75 group-hover:opacity-75 cursor-pointer object-cover h-full w-full" src={BASE_POST_IMAGE_URL + image} alt="Post" />
            ...
            <Dialog open={open} onClose={() => setOpen(false)} maxWidth='xl'>
                <div className="flex sm:flex-row flex-col max-w-7xl">
                    ...
                    <div className="relative flex items-center justify-center bg-black sm:h-[90vh] w-full" onDoubleClick={setLike}>
                        <img draggable="false" className="object-contain h-full w-full" src={BASE_POST_IMAGE_URL + image} alt="post" />
                        {likeEffect &&
                    ...
                    <div className="flex flex-col justify-between border w-full max-w-xl rounded bg-white">
                        ...
                        <div className="p-4 w-full flex-1 max-h-[63vh] overscroll-x-hidden overflow-y-auto">
                            ...
                            <div className="flex items-start">
                                <Link to={`/${postedBy.username}`} className="w-12"><img draggable="false" className="w-9 h-9 rounded-full object-cover" src={BASE_PROFILE_IMAGE_URL + postedBy.avatar} alt="avatar" /></Link>
                            ...
                            {comments.map((c) => (
                                <div className="flex items-start space-x-1 mb-3" key={c._id}>
                                    <Link to={`/${c.user}`}><img draggable="false" className="w-9 h-9 rounded-full object-cover mr-2.5" src={BASE_PROFILE_IMAGE_URL + c.user.avatar} alt="avatar" /></Link>
